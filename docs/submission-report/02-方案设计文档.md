...
![数据模型](assets/data-model-final-v3.png)
...
播报队列采用显式状态迁移规则，保证连续播报、暂停恢复和异常清理行为一致。

| 当前状态 | 事件 | 下一状态 | 处理规则 |
|---|---|---|---|
| 空闲 | 新任务 | 已排队 | 任务加入队尾 |
| 已排队 | 取队首任务 | 正在播报 | 锁定当前任务并启动音频 |
| 正在播报 | 当前任务完成且队列非空 | 已排队 | 自动处理下一任务 |
| 正在播报 | 当前任务完成且队列为空 | 空闲 | 释放当前播放资源 |
| 正在播报 | 用户暂停 | 已暂停 | 保留任务上下文与播放进度 |
| 已暂停 | 用户恢复 | 正在播报 | 从保存进度继续播放 |
| 已排队 | 清空待播 | 空闲 | 删除全部待播任务 |
| 正在播报 / 已暂停 | 中断并清空 | 空闲 | 终止当前任务、清空待播并释放资源 |
| 正在播报 / 已暂停 | 资源异常 | 空闲 | 记录错误、终止任务、清空队列并释放资源 |

新任务在播报或暂停期间仅追加至队尾，不改变当前播报状态。
### 4.3 运营指挥中心模块（核心模块三）

#### 4.3.1 数据聚合与展示流程

![运营数据](assets/command-center-nature.png)
...
![部署架构](assets/deployment-nature.png)
...
![数据初始化](assets/seed-init-nature.png)
