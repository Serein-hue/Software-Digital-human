...
| 交付层 | GitHub Pages、本地完整服务栈 | 支持静态展示与完整比赛演示部署 |
...
sequenceDiagram
  participant U as 游客
  participant UI as 导览终端
  participant API as 业务 API
  participant R as RAG
  participant F as Fay
...
系统将文字回答、TTS 音频、viseme 序列和前端渲染整合为统一播报链路，实现语音与嘴型的同步表现。
...
```mermaid
flowchart LR
  GH[自动构建] --> P[GitHub Pages]
  V[Web / Kiosk] --> B[business-api :8001]
  A[管理后台] --> D[admin-api :8002]
  M[微信小程序] --> B
  B --> R[RAG :5010]
  D --> R
  D --> F[Fay :5000]
  ...
  class GH,P,V,A,M,B,D,R,F,W,C,DB,CH node
```

*图 4　部署架构*

平台支持静态前端发布与完整服务栈部署。完整演示环境统一启动业务 API、管理 API、RAG、Fay、WebSocket 与 MCP 服务，并通过配置文件管理端口和服务地址。
