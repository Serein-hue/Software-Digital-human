# 灵山胜境智慧景区AI数字人导览平台 — 方案设计文档

> **文档版本**：v1.0  
> **编制日期**：2026年6月30日  
> **文档定位**：概要设计与详细设计方案，覆盖系统架构、技术选型、数据库、核心模块、接口、部署  
...
系统采用经典的四层架构，自顶向下分别为用户表现层、业务服务层、AI与知识服务层、数据持久层。各层职责明确，层间通过HTTP RESTful API通信。

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                             用户表现层 (Presentation)                         │
│  ┌──────────────────────┐  ┌──────────────────┐  ┌──────────────────────┐  │
│  │  C端导游页 (Web)     │  │  B端管理后台(Web) │  │  微信小程序          │  │
│  │  /guide /kiosk       │  │  /dashboard /kb   │  │  guide /spot-detail  │  │
│  │  React 19 + Vite     │  │  React 19 + Vite  │  │  原生 WXML+WXSS     │  │
│  └──────────┬───────────┘  └─────────┬────────┘  └──────────┬───────────┘  │
└─────────────┼────────────────────────┼───────────────────────┼──────────────┘
...
│                             业务服务层 (Service)                              │
│  ┌─────────────────────────────┐  ┌──────────────────────────────────────┐  │
│  │  导游业务 API (business-api) │  │  管理后台 API (admin-api)             │  │
│  │  FastAPI :8001              │  │  FastAPI :8002                        │  │
│  │  景点/路线/会话/消息/RAG代理 │  │  知识库/数字人配置/工单/分析/认证     │  │
│  └──────────┬──────────────────┘  └───────────┬──────────────────────────┘  │
└─────────────┼─────────────────────────────────┼──────────────────────────────┘
              │ HTTP                            │ HTTP
              ▼                                 ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                          AI与知识服务层 (AI & Knowledge)                     │
│  ┌────────────────────────┐  ┌──────────────────────┐  ┌────────────────┐  │
│  │  RAG 知识库服务        │  │  数字人引擎 (Fay)    │  │  LLM 网关       │  │
│  │  Flask :5010           │  │  :5000               │  │  DeepSeek API   │  │
│  │  语义检索/问答/入库/   │  │  Live2D渲染/         │  │                 │  │
│  │  Fallback/鉴权         │  │  TTS/ASR/口型同步    │  │                 │  │
│  └───────────┬────────────┘  └──────────────────────┘  └────────────────┘  │
└──────────────┼──────────────────────────────────────────────────────────────┘
               │
               ▼
...
│                             数据持久层 (Data)                                │
│  ┌────────────────────┐  ┌────────────────────┐  ┌──────────────────────┐  │
│  │  SQLite (业务数据)  │  │  ChromaDB (向量库)  │  │  JSON文件 (资料源/   │  │
│  │  spots/routes/      │  │  embedding向量+     │  │  QA对/种子数据)      │  │
│  │  sessions/messages  │  │  文档chunk          │  │                     │  │
│  └────────────────────┘  └────────────────────┘  └──────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│    基于关键词匹配判断查询意图                      │
│    (spot_fact / ticket / schedule / route /       │
│     family / service / notice / facility /        │
│     culture / general)                            │
│    自动推导 domain 过滤条件                       │
└──────────────────────┬──────────────────────────┘
                       │ intent_info（intent + domains）
                       ▼
┌─────────────────────────────────────────────────┐
│ 2. 查询向量化 (Query Embedding)                  │
│    调用 bge-small-zh-v1.5 输出 384维向量          │
│    设备：CPU（本地推理≈5-15ms）                   │
└──────────────────────┬──────────────────────────┘
                       │ query_embedding (384维 float[])
                       ▼
┌─────────────────────────────────────────────────┐
│ 3. 向量检索 (Vector Search)                      │
│    ChromaDB cosine相似度搜索                     │
│    top_k（默认5，最大20）+ 元数据过滤              │
│    返回：文本chunk + 相似度score + metadata       │
└──────────────────────┬──────────────────────────┘
                       │ contexts_raw (得分从高到低)
                       ▼
┌─────────────────────────────────────────────────┐
│ 4. Fallback 判定                                  │
│    ├─ score ≥ 0.50 → answerable=True             │
│    ├─ 0.30 ≤ score < 0.50 → answerable=False     │
│    │  （低分兜底，返回安全回复）                    │
│    └─ score < 0.30 → answerable=False             │
│       （无相关知识，返回礼貌拒绝）                   │
│    额外检查：敏感词过滤（手机号/身份证号）            │
└──────────────────────┬──────────────────────────┘
                       │ fallback_info（answerable / reason / safe_reply）
                       ▼
┌─────────────────────────────────────────────────┐
│ 5. 引用构建 (Citation Builder)                    │
│    从上下文提取引用摘要（各cquote 前200字）        │
│    + doc_id / source_name                        │
└──────────────────────┬──────────────────────────┘
                       │ citations[]
                       ▼
┌─────────────────────────────────────────────────┐
│ 6. 可选：LLM生成 (Answer Generation)              │
│    将contexts作为prompt上下文 + 用户问题          │
│    调用 DeepSeek API 生成自然语言回答              │
│    输出：answer_text + token_count               │
└──────────────────────┬──────────────────────────┘
                       │ 最终响应（含 answerable /
                       │ contexts / citations / fallback
                       ▼
```
...
鉴权方式：
Bearer Token（除 /health 外所有接口需要）
RAG默认Token：dev-token-123456（可通过 RAG_API_KEY 环境变量覆盖）
...
| 方法 | 路径 | 鉴权 | 说明 | 请求参数 | 返回数据 |
|------|------|:----:|------|----------|----------|
| GET | /api/v1/rag/health | ❌ | 健康检查 | — | {status, timestamp} |
| GET | /api/v1/rag/stats | ✅ | 知识库统计 | — | {vectors, provider, embedding_model, ...} |
| POST | /api/v1/rag/ingest | ✅ | 文档入库 | {filepath, metadata, chunk_size, overlap} | {job_id, success, chunks, elapsed} |
| POST | /api/v1/rag/query | ✅ | 语义检索 | {query, top_k, filters} | {answerable, contexts, citations, fallback, latencyMs} |
| POST | /api/v1/rag/answer | ✅ | 检索+LLM生成 | {query, top_k} | {answerable, answer, contexts, citations, tokens, latencyMs} |
| POST | /api/v1/rag/rebuild | ✅ | 重建索引 | — | {success, message} |
| POST | /api/v1/rag/qa | ✅ | 录入问答对 | {question, answer, source, domain} | qa对象 |
| GET | /api/v1/rag/qa | ✅ | 问答对列表 | page, page_size | {items, pagination} |
| POST | /api/v1/rag/sources | ✅ | 登记资料源 | {name, filepath, domain, description, tags} | source对象 |
| GET | /api/v1/rag/sources | ✅ | 资料源列表 | page, page_size | {items, pagination} |
...
│  │                                                              │      │
│  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐         │      │
│  │  │ 前端   │ │业务API│ │管理API│ │RAG    │ │数字人  │         │      │
│  │  │:5173  │ │:8001  │ │:8002  │ │:5010  │ │:5000  │         │      │
│  │  │(Vite) │ │FastAPI│ │FastAPI│ │Flask  │ │Fay    │         │      │
│  │  └──┬────┘ └──┬────┘ └──┬────┘ └──┬────┘ └──┬────┘         │      │
│  │     │         │         │         │         │              │      │
│  │     └─────────┴──HTTP───┴─────HTTP┴──HTTP───┘              │      │
│  │                                                              │      │
│  │  ┌──────────────────┐  ┌──────────────┐  ┌──────────────┐   │      │
│  │  │  SQLite 数据库    │  │ ChromaDB磁盘  │  │ JSON文件存储   │   │      │
│  │  │ app.db           │  │ chroma_db/   │  │ rag/data/     │   │      │
│  │  └──────────────────┘  └──────────────┘  └──────────────┘   │      │
│  └──────────────────────────────────────────────────────────────┘      │
└────────────────────────────────────────────────────────────────────────┘
```
...
| 服务 | 启动命令 | 端口 | 依赖 |
|------|----------|:----:|------|
| 前端开发 | `cd frontend && npm run dev` | 5173 | Node.js 18+ |
| 前端构建 | `cd frontend && npm run build` | 静态文件 | Node.js 18+ |
| 业务API | `cd services/scenic-dh-business-api && uvicorn app.main:app --port 8001` | 8001 | Python 3.10+, SQLite, RAG服务(可选) |
| 管理API | `cd services/scenic-dh-admin-api && uvicorn app.main:app --port 8002` | 8002 | Python 3.10+, SQLite, RAG服务 |
| RAG服务 | `cd rag && HF_ENDPOINT=https://hf-mirror.com python run_rag_server.py` | 5010 | Python 3.10+, ChromaDB, Embedding模型 |
| 数字人 | `python fay_booter.py` 或 `python fay_lite_server.py` | 5000 | Python 3.10+, 音频设备, TTS密钥 |
