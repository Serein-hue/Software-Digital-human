...
async def create_broadcast(body: BroadcastRequest, request: Request):
    """创建播报 — 通过 Fay /transparent-pass 发送到数字人"""
    ...
    try:
        payload = {
            "user": "System",
            "text": f"{body.speaker}说：{body.text}",
        }
        if body.queue:
            payload["queue"] = True
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                f"{settings.FAY_CORE_URL}/transparent-pass",
                json=payload,
            )
            if resp.ok:
                fay_success = True
                ...
                fay_message = f"Fay HTTP {resp.status_code}"
    ...
    return err(502, f"Fay broadcast failed: {fay_message}", trace_id=trace_id)
...
def list_broadcasts(request: Request):
    """查看播报历史"""
    trace_id = request.state.trace_id
    return ok({"items": list(reversed(_BROADCASTS))}, trace_id=trace_id)
